                    ...
                    <div className="py-24 md:pb-32 lg:pb-36 lg:pt-72">
                        <div className="relative z-10 mx-auto flex max-w-7xl flex-col px-6 lg:block lg:px-12">
                            <div className="mx-auto max-w-lg text-center lg:ml-0 lg:max-w-xl lg:text-left">
                                <div className="inline-flex items-center gap-2 rounded-full border border-[var(--accent-soft)] bg-[var(--surface-solid)]/80 backdrop-blur-md px-4 py-2 text-sm font-semibold text-[var(--accent)] shadow-[0_0_15px_var(--accent-soft)] mb-6">
                                    <span className="status-pulse" />
                                    Next-Gen Interpretation
                                </div>
                                <h1 className="mt-4 max-w-2xl text-balance text-5xl md:text-6xl lg:mt-6 xl:text-7xl font-extrabold tracking-tight">Interpret VCFs <span className="text-transparent bg-clip-text bg-gradient-to-br from-[var(--accent)] to-cyan-300">10x Faster</span></h1>
                                <p className="mt-8 max-w-2xl text-balance text-lg lg:text-xl text-[var(--text-secondary)] font-medium">Clinical-grade variant prioritization powered by deterministic pipelines and explained by an autonomous Gemini Copilot.</p>

                                <div className="mt-12 flex flex-col items-center justify-center gap-4 sm:flex-row lg:justify-start">
                                    <Button
                                        asChild
                                        size="lg"
                                        className="h-14 rounded-full pl-6 pr-4 shadow-[0_0_20px_rgba(56,189,248,0.3)] hover:shadow-[0_0_30px_rgba(56,189,248,0.5)] transition-all">
                                        <Link href="#upload-section">
                                            <span className="text-base text-nowrap font-bold uppercase tracking-wider">Start Analysis</span>
                                    ...
                                    <Button
                                        ...
                                        size="lg"
                                        variant="ghost"
                                        className="h-14 rounded-full px-6 text-base font-semibold hover:bg-zinc-950/5 dark:hover:bg-white/5">
                                        <Link href="#features">
                                            <span className="text-nowrap tracking-wide">Explore Features</span>
                                        </Link>
                                    </Button>
                                </div>
                            </div>
                        ...
                        <div className="aspect-[2/3] absolute inset-1 overflow-hidden rounded-3xl border border-black/10 sm:aspect-video lg:rounded-[3rem] dark:border-white/5">
                            <video
                                ...
                                muted
                                playsInline
                                className="size-full object-cover opacity-50 invert dark:opacity-35 dark:invert-0 dark:lg:opacity-60 contrast-125 mix-blend-screen"
                                src="https://ik.imagekit.io/lrigu76hy/tailark/dna-video.mp4?updatedAt=1745736251477"></video>
                        </div>
                    </div>
                </section>
                <section className="bg-background pb-2 border-b border-[var(--line)]">
                    <div className="group relative m-auto max-w-7xl px-6">
                        <div className="flex flex-col items-center md:flex-row">
                            <div className="md:max-w-44 md:border-r border-[var(--line)] md:pr-6 shrink-0 py-4">
                                <p className="text-center md:text-end text-xs font-bold uppercase tracking-[0.2em] text-[var(--text-muted)]">Powering insights with</p>
                            </div>
                            <div className="relative py-6 w-full md:flex-1 h-20 flex items-center">
                                <InfiniteSlider
                                    durationOnHover={20}
                                    duration={40}
                                    gap={60} className="w-full h-8 px-4">
                                        <div className="flex items-center text-xl font-black text-white/30 tracking-widest px-4 hover:text-white/60 transition-colors">CLINVAR</div>
                                        <div className="flex items-center text-xl font-black text-white/30 tracking-widest px-4 hover:text-white/60 transition-colors">GNOMAD</div>
                                        <div className="flex items-center text-xl font-black text-white/30 tracking-widest px-4 hover:text-white/60 transition-colors">ENSEMBL VEP</div>
                                        <div className="flex items-center text-xl font-black text-white/30 tracking-widest px-4 hover:text-white/60 transition-colors">PANELAPP</div>
                                        <div className="flex items-center text-xl font-black text-white/30 tracking-widest px-4 hover:text-white/60 transition-colors">OMIM</div>
                                        <div className="flex items-center text-xl font-black text-white/30 tracking-widest px-4 hover:text-white/60 transition-colors">HPO</div>
                                        <div className="flex items-center text-xl font-black text-white/30 tracking-widest px-4 hover:text-white/60 transition-colors">PUBMED</div>
                                        <div className="flex items-center text-xl font-black text-white/30 tracking-widest px-4 hover:text-white/60 transition-colors">GEMINI 2.0</div>
                                </InfiniteSlider>

                                <div className="bg-gradient-to-r from-background absolute inset-y-0 left-0 w-20 pointer-events-none z-10"></div>
                                <div className="bg-gradient-to-l from-background absolute inset-y-0 right-0 w-20 pointer-events-none z-10"></div>
                                <ProgressiveBlur
...
const menuItems = [
    { name: 'Upload VCF', href: '#upload-section' },
